...
query :: (Labeled label SQLite :> es) => (ToRow q, FromRow r) => Connection label mode -> Query -> q -> Eff es [r]
query conn q params = unsafeEff_ $ S.query conn.getConn.getConn q params

query_ :: (Labeled label SQLite :> es) => (FromRow r) => Connection label mode -> Query -> Eff es [r]
query_ conn q = unsafeEff_ $ S.query_ conn.getConn.getConn q

queryWith :: (Labeled label SQLite :> es) => (ToRow q) => RowParser r -> Connection label mode -> Query -> q -> Eff es [r]
queryWith parser conn q params = unsafeEff_ $ S.queryWith parser conn.getConn.getConn q params

queryWith_ :: (Labeled label SQLite :> es) => RowParser r -> Connection label mode -> Query -> Eff es [r]
queryWith_ parser conn q = unsafeEff_ $ S.queryWith_ parser conn.getConn.getConn q

queryNamed :: (Labeled label SQLite :> es) => (FromRow r) => Connection label mode -> Query -> [NamedParam] -> Eff es [r]
...
fold_ :: forall label row a es mode. (Labeled label SQLite :> es) => (FromRow row) => Connection label mode -> Query -> a -> (a -> row -> Eff es a) -> Eff es a
fold_ conn q initialState action =
  unsafeEffWithUnlift @label \unlift -> do
    S.fold_ conn.getConn.getConn q initialState \a row ->
      unlift $ action a row

foldNamed :: forall label row a es mode. (Labeled label SQLite :> es) => (FromRow row) => Connection label mode -> Query -> [NamedParam] -> a -> (a -> row -> Eff es a) -> Eff es a
foldNamed conn q params initialState action =
  unsafeEffWithUnlift @label \unlift -> do
    S.foldNamed conn.getConn.getConn q params initialState \a row ->
      unlift $ action a row

execute :: (Labeled label SQLite :> es) => (ToRow q) => Connection label 'Write -> Query -> q -> Eff es ()
execute conn q params = unsafeEff_ $ S.execute conn.getConn.getConn q params

execute_ :: (Labeled label SQLite :> es) => Connection label 'Write -> Query -> Eff es ()
execute_ conn q = unsafeEff_ $ S.execute_ conn.getConn.getConn q

executeMany :: (Labeled label SQLite :> es) => (ToRow q) => Connection label 'Write -> Query -> [q] -> Eff es ()
executeMany conn q params = unsafeEff_ $ S.executeMany conn.getConn.getConn q params

executeNamed :: (Labeled label SQLite :> es) => Connection label 'Write -> Query -> [NamedParam] -> Eff es ()
executeNamed conn q params = unsafeEff_ $ S.executeNamed conn.getConn.getConn q params

withTransaction :: forall label a es mode. (Labeled label SQLite :> es) => Connection label mode -> Eff es a -> Eff es a
withTransaction conn action =
  unsafeEffWithUnlift @label \unlift -> do
    S.withTransaction conn.getConn.getConn $ unlift action

withImmediateTransaction :: forall label a es. (Labeled label SQLite :> es) => Connection label 'Write -> Eff es a -> Eff es a
withImmediateTransaction conn action =
  unsafeEffWithUnlift @label \unlift -> do
    S.withImmediateTransaction conn.getConn.getConn $ unlift action

withExclusiveTransaction :: forall label a es. (Labeled label SQLite :> es) => Connection label 'Write -> Eff es a -> Eff es a
withExclusiveTransaction conn action =
  unsafeEffWithUnlift @label \unlift -> do
    S.withExclusiveTransaction conn.getConn.getConn $ unlift action

withSavepoint :: forall label a es. (Labeled label SQLite :> es) => Connection label 'Write -> Eff es a -> Eff es a
withSavepoint conn action =
  unsafeEffWithUnlift @label \unlift -> do
    S.withSavepoint conn.getConn.getConn $ unlift action

openStatement :: (Labeled label SQLite :> es) => Connection label 'Write -> Query -> Eff es Statement
...
withStatement :: forall label a es. (Labeled label SQLite :> es) => Connection label 'Write -> Query -> (Statement -> Eff es a) -> Eff es a
